...
from main_main import ya_daun
from max_privacy import TOKEN
...
async def process_metro_time(message: types.Message, state: FSMContext):
    try:
        metro_time_to = int(message.text.strip())
        metro_time_from = 1
...
async def process_park_time(message: types.Message, state: FSMContext):
    try:
        park_time_to = int(message.text.strip())
        park_time_from = 1
...
async def process_school_time(message: types.Message, state: FSMContext):
    try:
        school_time_to = int(message.text.strip())
        school_time_from = 1
...
async def process_clinic_time(message: types.Message, state: FSMContext):
    try:
        clinic_time_to = int(message.text.strip())
        clinic_time_from = 1
...
async def process_kindergarten_time(message: types.Message, state: FSMContext):
    try:
        kindergarten_time_to = int(message.text.strip())
        kindergarten_time_from = 1
        await state.update_data(kindergarten_time_from=kindergarten_time_from, kindergarten_time_to=kindergarten_time_to)
        await state.set_state(Form.bar_time)
        await message.answer("Введите диапазон времени до бара (например, 5 30):", reply_markup=start_over_button())
    except ValueError:
        await message.answer("Некорректный ввод. Пожалуйста, введите число.")

@dp.message(Form.bar_time)
async def process_bar_time(message: types.Message, state: FSMContext):
    try:
        bar_time_input = message.text.split()
        if len(bar_time_input) != 2:
            raise ValueError("Введите два числа через пробел.")
        bar_time_from, bar_time_to = map(int, bar_time_input)
        await state.update_data(bar_time_from=bar_time_from, bar_time_to=bar_time_to)
        ...
        await message.answer("Пожалуйста, введите два числа через пробел для времени до бара.")
...
async def process_parking(callback: types.CallbackQuery, state: FSMContext):
    ...
    model_input = {
        ...
        'time_to_subway': f"{user_data.get('metro_time_to')} {user_data.get('metro_time_to')}",
        'time_to_park': f"{user_data.get('park_time_to')} {user_data.get('park_time_to')}",
        'time_to school': f"{user_data.get('school_time_to')} {user_data.get('school_time_to')}",
        'time_to_hospital': f"{user_data.get('clinic_time_to')} {user_data.get('clinic_time_to')}",
        'time_to_kindergarten': f"{user_data.get('kindergarten_time_to')} {user_data.get('kindergarten_time_to')}",
        'time_to_bar': f"{user_data.get('bar_time_to')} {user_data.get('bar_time_to')}",
        'parking_space': f"{user_data.get('parking')}"
    }
    u = ya_daun(model_input)
    await callback.message.edit_text(f"Средняя цена квартиры, учитывая все Ваши запросы, составляет: {'{:,}'.format(int((u // 1000) * 1000)).replace(',', ' ')} ₽")
